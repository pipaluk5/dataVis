...
server <- function(input, output) {
    ...
    output$ratingPlot <- renderPlotly({
        ...
        if (is.null(event.data)) {
            ...
            plot_ly(ice.cream.df, y = ~fct_reorder(flavors,rating), x = ~rating, type = "bar",  orientation ='h',
                    text = ~paste("Flavor:", flavors),autorange="reversed") %>%
                layout(title = paste("Ice Cream Ratings Given by Flavor"))
        } else {
            ice.cream <- ice.cream.df[ice.cream.df$images %in% event.data$key,]
            plot_ly(ice.cream, y = ~fct_reorder(flavors,rating), x = ~rating, type = "bar", orientation ='h',
                    text = ~paste("Flavor:", flavors), key = ~images, source = "imgLink",autorange="reversed") %>%
                layout(title = paste("Ice Cream Ratings Given by Flavor"))
        }
    })
}
